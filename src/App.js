...
function App() {
  ...
  return (
    <>
      ...
      {/* 2. use the state variable based on the tenary operation condition */}
      {showForm ? (
        <NewFactFrorm setFacts={setFacts} setShowForm={setShowForm} />
      ) : null}
  ...
  return (
    <header className="header">
      ...
      <button
        ...
        //3. update the state variable(tell the web to re-render the page)  whenever we need to render something on the screen ,use state
...
function NewFactFrorm({ setFacts, setShowForm }) {
  const [text, setText] = useState("");
  const [source, setSource] = useState("http://example.com");
  const [category, setCategory] = useState("");
  const [isUploading, setIsUploading] = useState(false);
  const textLength = text.length;
  //1. prevent react reload the page by default
  async function handleSubmit(e) {
    e.preventDefault();
    console.log(text, source, category);
    //2. check if the text is valid, if so, create a neww fact
